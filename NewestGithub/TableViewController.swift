...
/*
 ...
 6.  In the Document Outline to the left of the Canvas, select your new Table View Controller.
 ...
 7.  In the Document Outline, select the Table View Cell under the Table View Controller.
 In the Attributes inspector, change the Style from Custom to Subtitle
 and change the Identifier to projectCell.

 8.  In the Document Outline, select the Table View Controller.
 Editor -> Embed In -> Navigation Controller
 Your Outline View should now have a Navigation Controller Scene and a Table View Controller Scene.
 
 9.  In the Document Outline, select the Navigation Item under the Table View.
 ...
 In the Document Outline, select the new view controller.
 In the Identity inspector, change the class of the new view controller
 from UIViewController to WebViewController.
 
 13.  In the Document Outline, select the projectCell under the table view.
...
class TableViewController: UITableViewController {
    ...
    func updateModelAndUI() {
        let urlString: String = "https://api.github.com/search/repositories"
            + "?q=SF18AS01-"      //Search for repositories that have SF18AS01- in their name.
            + "+in:name"
...
func complete(filename: URL?, response: URLResponse?, error: Error?) {
    ...
    for project in projects {
        ...
        if let range: Range = full_name.range(of: "SF18AS01-") {
            //Chop off the leading "SF18AS01-".
